...
const init = async() => {
    ...
    const web3Eth = new Web3(
        new Web3.providers.WebsocketProvider(
            process.env.ETH_PROVIDER_URL_WSS
        )
    );

    const web3Bsc = new Web3(process.env.BNC_PROVIDER_URL); 
    ...
    .on('data', async event => {
        ...
        console.log(`Event on ETH happened: ${step}`);
        
        const tx = step == 0 
            ? bridgeBnc.methods.unlock(owner, nonce, amount, signature) 
            : bridgeBnc.methods.mint(owner, nonce, amount, signature);
        ...
        console.log(`BNC Transaction hash: ${receipt.transactionHash}`);
        console.log(`
            Processed transfer:
            - from ${owner} 
